...
void ruby_mic_enable(int en, int shift)
{
	...
	if (en) {
		...
		if (rc) {
			pr_err("%s: unable to set 8058_s4 voltage"
					" to 2.6 V\n", __func__);
			goto vreg_fail;
		}
		...
		if (rc) {
			pr_err("%s: failed on vreg_enabled %d\n",
					__func__, rc);
			goto vreg_fail;
		}
		...
		if (rc) {
			pr_err("%s: failed on vreg_disable %d\n",
					__func__, rc);
			goto vreg_fail;
		}
...
void ruby_snddev_tx_pamp_on(int en)
{
	...
	if (rc) {
		pr_err("%s: unable to set 8058_l5 voltage\n", __func__);
		goto vreg_fail;
	}

	rc = regulator_enable(vreg_l5);
	if (rc) {
		pr_err("%s: unable to enable 8058_l5\n", __func__);
		goto vreg_fail;
	}
...
void ruby_usb_headset_on(int en)
{
	...
	if (en) {
		...
		if (rc < 0) {
			pr_err("%s: regulator_set_voltage(ncp) failed"
					"(%d)\n", __func__, rc);
			goto vreg_fail;
		}
