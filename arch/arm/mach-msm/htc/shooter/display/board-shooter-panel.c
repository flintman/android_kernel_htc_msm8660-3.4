...
static int mipi_dsi_panel_power(int on)
{
	...
	if (!dsi_power_on) {
		...
		if (isorise == 0)
			...
			ret = regulator_set_voltage(l1_3v, 3200000, 3200000);
		if (ret) {
...
static struct platform_device mipi_dsi_shooter_panel_device = {
	.name = "mipi_shooter",
	.id = 0,
};
...
int mipi_cmd_shooter_qhd_pt_init(void);

void __init shooter_init_fb(void)
{
	msm8x60_set_display_params("mipi_shooter", "hdmi_msm");
	platform_device_register(&msm_fb_device);
	platform_device_register(&mipi_dsi_shooter_panel_device);
