...
static int mipi_shooter_send_cmd(struct dsi_cmd_desc *cmd, unsigned int len, bool clk_ctrl)
{
	int ret = 0;
...
static struct msm_panel_common_pdata *mipi_shooter_pdata;

static int mipi_shooter_lcd_init(void);
...
void mipi_shooter_panel_type_detect(void)
{
...
static int mipi_shooter_lcd_on(struct platform_device *pdev)
{
	...
	mipi  = &mfd->panel_info.mipi;
        ...
        mipi_shooter_send_cmd(mipi_power_on_cmd, mipi_power_on_cmd_size, false);
...
static int mipi_shooter_display_on(struct platform_device *pdev)
{
  struct msm_fb_data_type *mfd;
  unsigned int val = 0;
  ...
  mipi_shooter_send_cmd(novatek_display_on_cmds, ARRAY_SIZE(novatek_display_on_cmds), true);
...
static int mipi_shooter_lcd_off(struct platform_device *pdev)
{
  struct msm_fb_data_type *mfd;
...
static int mipi_shooter_display_off(struct platform_device *pdev)
{
  struct msm_fb_data_type *mfd;

  mfd = platform_get_drvdata(pdev);

  mipi_shooter_send_cmd(mipi_power_off_cmd, mipi_power_off_cmd_size, true);
...
static void mipi_shooter_set_backlight(struct msm_fb_data_type *mfd)
{
  
        mipi_dsi_set_backlight(mfd, mfd->bl_level);
}

static int mipi_shooter_lcd_probe(struct platform_device *pdev)
{
	if (pdev->id == 0) {
		mipi_shooter_pdata = pdev->dev.platform_data;
		return 0;
	}
...
static struct msm_fb_panel_data shooter_panel_data = {
	.on		= mipi_shooter_lcd_on,
        .off		= mipi_shooter_lcd_off,
        .set_backlight  = mipi_shooter_set_backlight,
	.late_init = mipi_shooter_display_on,
	.early_off = mipi_shooter_display_off,
};
					...
					u32 channel, u32 panel)
{
	...
	ret = mipi_shooter_lcd_init();
	if (ret) {
		pr_err("mipi_shooter_lcd_init() failed with ret %u\n", ret);
		return ret;
	}

	pdev = platform_device_alloc("mipi_shooter", (panel << 8)|channel);
	if (!pdev)
		return -ENOMEM;
...
static int mipi_shooter_lcd_init(void)
{
        mipi_shooter_panel_type_detect();
